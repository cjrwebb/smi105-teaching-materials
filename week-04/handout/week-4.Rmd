---
title: "Data Visualisation Week 4: Box plots and ridgeline plots"
...
```{r, eval = FALSE}
ggplot(data = taylor) +
  aes(x = tempo, colour = album) +
  geom_density()
```

This doesn't work as well. Comparing two overlapping density curves is fairly straightforward to do, but comparing *eleven* is tricky; it's not as easy to identify the overall trends for all eleven (It's also worth noting this comparison is a bit easier than it is for other artists - try it with the other data we've loaded). As ever, a good way to deal with eleven lots of information in one graph is to turn it into eleven graphs, with the facet_wrap() command.
...
## Moving to Fleetwood Mac

What happens when we're looking at a categorical variable with more than four levels? Fleetwood Mac released a lot of albums, let's look at them. 
...
```{r, eval = FALSE}
ggplot(data = fleet) +
  aes(fct_reorder(album, ymd(release_date)), 
      tempo) +
  geom_boxplot() + 
  coord_flip()
```
...
- the first thing in the fct_reorder() bracket is the variable we want to put on that axis: album. But while we'd normally close the bracket here, instead we have a comma, anticipating...
...
- which we need to surround with ymd(), to indicate it's a date of the format YYYY-MM-DD.
...
```{r, eval = FALSE}
ggplot(data = fleet) +
  aes(fct_reorder(album, tempo), tempo) +
  geom_boxplot() +
  coord_flip()
```

The difference here is that earlier we were reordering according to release_date; here, we're reordering according to tempo.
...
ggplot(data = fleet) +
  aes(x = tempo, 
      y = fct_reorder(album, ymd(release_date))) +
  geom_density_ridges()
```
...
ggplot(data = fleet) +
  aes(x = tempo, 
      y = fct_rev(fct_reorder(album, 
                          ymd(release_date)))) +
  geom_density_ridges() 
```

This is clumsy, but it works. What we've done is we've wrapped the existing y aesthetic mapping -- reorder(album_name, ymd(album_release_date)) -- which included a lot more brackets than we're used to, in *yet another* set of brackets, preceded by the **fct_rev()** command. fct_rev() *reverses* the order of levels in a factor, so it's often useful in this kind of context.
...
ggplot(data = fleet) +
  aes(x = tempo, 
      y = fct_reorder(album, popularity)) +
  geom_density_ridges() 
...
Please also show me something interesting about how a continuous variable varies by a categorical variable in one of the bits of Spotify data, and post it to the discussion board before next week's session.