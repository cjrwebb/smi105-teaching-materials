...
- you'll note that the variable *release_date* is wrapped in ymd(). This is so we're declaring to R that that values of that variable are of the format *year-month-day*. Try swapping ymd() for as.factor() and see what happens. (If this doesn't work, you probably didn't load the **lubridate** package.)
...
So. One issue with this data so far is that, while you might know the order that Taylor Swift's albums came out in, not everyone does. If I looked at that previous graph without that knowledge, I wouldn't know whether her more recent albums were more popular or not. At the moment, the albums are just in alphabetical order. Let's change that. 
...
- what about for Arctic Monkeys? (please ensure that the albums are in chronological order)
